...
class LudHuffManTree {
    ...
    LudHuffManTree(HashMap<Integer, Integer> map) {
        this.map = map;
    ...
    public class Node implements Comparable<Node> {
        ...
        public int compareTo(Node otherNode) {
            if (freq > otherNode.freq) return 1;
            if (freq < otherNode.freq) return -1;
            return 0;
        }
        ...
        public void setBinRepr(String binRepr) {
            if (asciiChar != -1) {
                ...
                // System.out.println("Gave " + asciiChar + " representation " + binRepr);
            }
            if (leftChild != null) {
    ...
    private void populateDictionary() {
        root.setBinRepr(""); // starter rekursivt kall fra rootnoden, der alle legger til sin binrepresentasjon i dictionary
    }

    /**
     * Encodes a String with current Huffman encoding.
     * Is not tested for situations where String contains chars which are not in dictionary
     * @param text
     * @return
     */
